...
M.gradingform_simplefeedbackrubriceditor.addhandlers = function() {
    ...
    if (M.gradingform_simplefeedbackrubriceditor.eventhandler) M.gradingform_simplefeedbackrubriceditor.eventhandler.detach()
...
M.gradingform_simplefeedbackrubriceditor.disablealleditors = function() {
    var Y = M.gradingform_simplefeedbackrubriceditor.Y
    var name = M.gradingform_simplefeedbackrubriceditor.name
    Y.all('#simplefeedbackrubric-'+name+' .level').each( function(node) {M.gradingform_simplefeedbackrubriceditor.editmode(node, false)} );
    Y.all('#simplefeedbackrubric-'+name+' .description').each( function(node) {M.gradingform_simplefeedbackrubriceditor.editmode(node, false)} );
}
...
// handler for clicking on submit buttons within simplefeedbackrubriceditor element. Adds/deletes/rearranges criteria and/or levels on client side
M.gradingform_simplefeedbackrubriceditor.buttonclick = function(e, confirmed) {
    ...
    if (chunks.length == 3 && action == 'addcriterion') {
        ...
        M.gradingform_simplefeedbackrubriceditor.disablealleditors()
        M.gradingform_simplefeedbackrubriceditor.assignclasses(elements_str)
        M.gradingform_simplefeedbackrubriceditor.editmode(parent.all('.level').item(parent.all('.level').size()-1), true)
