...
/**
 * File contains definition of class MoodleQuickForm_simplefeedbackrubriceditor
 *
...
/**
 * Form element for handling simplefeedbackrubric editor
 *
 * The simplefeedbackrubric editor is defined as a separate form element. This allows us to render
 * criteria, levels and buttons using the simplefeedbackrubric's own renderer. Also, the required
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_simplefeedbackrubriceditor extends HTML_QuickForm_input {
    ...
    /** @var bool Message to display in front of the editor (that there exist grades on this simplefeedbackrubric being edited) */
    ...
    /**
     * Specifies that confirmation about re-grading needs to be added to this simplefeedbackrubric editor.
     * $changelevel is saved in $this->regradeconfirmation and retrieved in toHtml()
     *
     * @see gradingform_simplefeedbackrubric_controller::update_or_check_simplefeedbackrubric()
    ...
    public function toHtml() {
        global $PAGE;
        $html = $this->_getTabs();
        $renderer = $PAGE->get_renderer('gradingform_simplefeedbackrubric');
        ...
        $html .= $renderer->display_simplefeedbackrubric($data['criteria'], $data['options'], $mode, $this->getName());
        return $html;
    }
     ...
     * - if options not passed (i.e. we create a new simplefeedbackrubric) fills the options array with the default values
    ...
    protected function prepare_data($value = null, $withvalidation = false) {
        ...
        $totalscore = 0;
        $errors = array();
        $return = array('criteria' => array(), 'options' => gradingform_simplefeedbackrubric_controller::get_default_options());
        ...
        if ($withvalidation) {
            if (count($errors)) {
                ...
                foreach ($errors as $error => $v) {
                    $rv[] = get_string($error, 'gradingform_simplefeedbackrubric');
                }
    ...
    /**
     * Validates that simplefeedbackrubric has at least one criterion, at least two levels within one criterion,
